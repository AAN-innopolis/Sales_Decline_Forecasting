...
def create_temporal_features(df, target='sale_dollars'):
    ...
    for store, store_df in df_features.groupby('store'):
        ...
        for window in date_windows:
            ...
            # Avoid divide by zero in momentum calculation
            rolling_mean_nonzero = store_df[f'{target}_rolling_mean_{window}'].replace(0, np.nan)
            store_df[f'{target}_rel_momentum_{window}'] = (store_df[target] / rolling_mean_nonzero - 1) * 100
        
        # 3. Time since last purchase features
        store_df['days_since_last_purchase'] = (store_df.index - store_df.index.shift(1, freq='D')).days
        
        # 4. Sales decline features considering irregular data - with NaN handling
        # Handle NaN values safely in comparisons
        
        # Define decline relative to previous purchase
        lag_1d = store_df[f'{target}_lag_1d']
        store_df[f'{target}_decrease_prev'] = np.where(
            pd.notnull(store_df[target]) & pd.notnull(lag_1d),
            (store_df[target] < lag_1d).astype(int),
            0  # Default to 0 if either value is NaN
        )
        
        # Define decline relative to averages over different periods
        for period in ['7D', '30D']:
            rolling_mean = store_df[f'{target}_rolling_mean_{period}']
            store_df[f'{target}_decrease_{period[:-1].lower()}d_avg'] = np.where(
                pd.notnull(store_df[target]) & pd.notnull(rolling_mean),
                (store_df[target] < rolling_mean).astype(int),
                0
            )
        
        # Significant decline (more than 10% from 30-day average)
        rolling_mean_30d = store_df[f'{target}_rolling_mean_30D']
        store_df[f'{target}_significant_decrease'] = np.where(
            pd.notnull(store_df[target]) & pd.notnull(rolling_mean_30d),
            (store_df[target] < rolling_mean_30d * 0.9).astype(int),
            0
        )
        ...
        if len(store_df) >= 3:
            ...
            val_current = store_df[target]
            val_prev1 = store_df[target].shift(1)
            val_prev2 = store_df[target].shift(2)
            
            store_df['consecutive_decrease'] = np.where(
                pd.notnull(val_current) & pd.notnull(val_prev1) & pd.notnull(val_prev2),
                ((val_current < val_prev1) & (val_prev1 < val_prev2)).astype(int),
                0
            )
        else:
    ...
    # Fill NaN values in combined dataframe
    result_df = pd.concat(store_dfs, ignore_index=True)
    
    # Replace any remaining NaN values in the critical features with appropriate defaults
    cols_to_fill = [f'{target}_significant_decrease', f'{target}_decrease_prev', 
                   f'{target}_decrease_7d_avg', f'{target}_decrease_30d_avg']
...
def aggregate_by_store_date(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Create longitude and latitude columns as float type
    store_attributes['lon'] = 0.0  # Initialize as float
    store_attributes['lat'] = 0.0  # Initialize as float
    
    # Check if store_location column exists and has data
    if 'store_location' in store_attributes.columns and not store_attributes['store_location'].isna().all():
        try:
            # Handle different possible formats of store_location
            for idx, loc in store_attributes['store_location'].items():
                if loc is None:
                    continue
                ...
                if isinstance(loc, str):
                    try:
                        ...
                        if 'coordinates' in loc_dict:
                            store_attributes.at[idx, 'lon'] = float(loc_dict['coordinates'][0])
                            store_attributes.at[idx, 'lat'] = float(loc_dict['coordinates'][1])
                    except:
                        # If eval fails, try to handle it as JSON
                        try:
                            import json
                            ...
                            logger.warning(f"Could not parse store_location: {loc}")
                            
                elif isinstance(loc, dict):
                    # If it's already a dictionary
                    if 'coordinates' in loc:
                        store_attributes.at[idx, 'lon'] = float(loc['coordinates'][0])
                        store_attributes.at[idx, 'lat'] = float(loc['coordinates'][1])
        except Exception as e:
            logger.warning(f"Error extracting coordinates from store_location: {e}")
    ...
    # Create additional metrics - handle division by zero properly
    sale_bottles_nonzero = aggregated['sale_bottles'].replace(0, np.nan)
    sale_liters_nonzero = aggregated['sale_liters'].replace(0, np.nan)
    transaction_count_nonzero = aggregated['transaction_count'].replace(0, np.nan)
    
    aggregated['avg_price_per_bottle'] = aggregated['sale_dollars'] / sale_bottles_nonzero
    aggregated['avg_price_per_liter'] = aggregated['sale_dollars'] / sale_liters_nonzero
    aggregated['avg_items_per_transaction'] = aggregated['unique_items'] / transaction_count_nonzero
    
    # Calculate medians while explicitly dropping NaN values
    bottle_median = aggregated['avg_price_per_bottle'].dropna().median()
    liter_median = aggregated['avg_price_per_liter'].dropna().median()
    items_median = aggregated['avg_items_per_transaction'].dropna().median()
    
    # Use safe fallback values in case medians are NaN
    if pd.isna(bottle_median): bottle_median = 0.0
    if pd.isna(liter_median): liter_median = 0.0
    if pd.isna(items_median): items_median = 0.0
    
    # Fill missing values
    aggregated = aggregated.fillna({
        'avg_price_per_bottle': bottle_median,
        'avg_price_per_liter': liter_median,
        'avg_items_per_transaction': items_median
    })
